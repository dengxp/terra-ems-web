...
const EquipmentForm: React.FC<Props> = ({ visible, onCancel, onSuccess }) => {
    ...
    return (
        <ProModalForm
            title={state.operation === OperationEnum.EDIT ? '编辑用能设备' : '新增用能设备'}
            open={visible}
            onOpenChange={(open) => { if (!open) onCancel(); }}
            form={form}
        ...
        >
            ...
            <ProFormText name="name" label="设备名称" placeholder="请输入设备名称" rules={[{ required: true, message: '请输入设备名称' }]} />
            <ProFormText name="code" label="设备编码" placeholder="请输入设备编码" rules={[{ required: true, message: '请输入设备编码' }]}
                disabled={state.operation === OperationEnum.EDIT} />
            <ProFormText name="type" label="设备类型" placeholder="请输入设备类型" />
            <ProFormText name="modelNumber" label="规格型号" placeholder="请输入规格型号" />
            <ProFormText name="manufacturer" label="生产厂商" placeholder="请输入生产厂商" />
            <ProFormDigit name="ratedPower" label="额定功率(kW)" placeholder="请输入额定功率" fieldProps={{ precision: 2 }} />
            <ProFormText name="location" label="安装位置" placeholder="请输入安装位置" />
            <ProFormTreeSelect name="energyUnitId" label="所属用能单元"
                request={async () => {
                    const res = await getEnabledEnergyUnitTree();
                    return res.data || [];
                }}
                fieldProps={{ fieldNames: { label: 'name', value: 'id', children: 'children' }, allowClear: true, placeholder: '请选择所属用能单元' }} />
