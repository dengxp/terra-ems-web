...
const GatewayForm: React.FC<Props> = ({ visible, onCancel, onSuccess }) => {
    ...
    useEffect(() => {
        if (visible) {
            if (state.operation === OperationEnum.EDIT) {
                ...
                form.resetFields();
                form.setFieldsValue({ status: 0, runStatus: 'UNKNOWN' });
            }
        }
    ...
    return (
        <ProModalForm
            title={state.operation === OperationEnum.EDIT ? '编辑网关' : '新增网关'}
            open={visible}
            onOpenChange={(open) => { if (!open) onCancel(); }}
            form={form}
        ...
        >
            ...
            <ProFormText name="name" label="网关名称" placeholder="请输入网关名称" rules={[{ required: true, message: '请输入网关名称' }]} />
            <ProFormText name="code" label="网关编码" placeholder="请输入网关编码" rules={[{ required: true, message: '请输入网关编码' }]}
                disabled={state.operation === OperationEnum.EDIT} />
            <ProFormText name="model" label="规格型号" placeholder="请输入规格型号" />
            <ProFormText name="manufacturer" label="生产厂商" placeholder="请输入生产厂商" />
            <ProFormText name="ipAddress" label="IP 地址" placeholder="请输入IP地址" />
            <ProFormText name="installLocation" label="安装位置" placeholder="请输入安装位置" />
